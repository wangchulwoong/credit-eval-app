...
def login_required(f):
    ...
    def decorated(*args, **kwargs):
        if 'username' not in session:
            # API 요청은 JSON으로, 일반 요청은 리다이렉트로
            if request.path.startswith('/api/'):
                return jsonify({'error': '세션이 만료되었습니다. 페이지를 새로고침하여 다시 로그인해주세요.'}), 401
    ...
    for attempt in range(3):
        try:
            ...
            if resp.status_code != 200:
                try:
                    err_data = resp.json()
                    msg = err_data.get('error', {}).get('message', f'HTTP {resp.status_code} 오류')
                except Exception:
                    msg = f'HTTP {resp.status_code} 오류'
                if resp.status_code in [429, 503] or 'high demand' in msg:
                    last_error = msg
                    continue
            ...
            if 'error' in result:
                ...
                if 'high demand' in msg or resp.status_code == 503:
                    last_error = msg
                    continue
                return jsonify({'error': msg}), 500
            ...
            last_error = '분석 시간이 초과되었습니다.'
            continue
        except Exception as e:
            last_error = str(e)
            continue

    return jsonify({'error': f'분석 실패: {last_error}. 잠시 후 다시 시도해주세요.'}), 503
