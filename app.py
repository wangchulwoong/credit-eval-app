...
def analyze():
    ...
    for attempt in range(3):  # 최대 3회 재시도
        try:
            if attempt > 0:
                time.sleep(5 * attempt)  # 2회차 5초, 3회차 10초 대기
            ...
            if 'error' in result:
                ...
                # 과부하 오류면 재시도, 그 외엔 즉시 반환
                if 'high demand' in msg or 'quota' in msg.lower() or '503' in str(resp.status_code):
                    last_error = msg
                    continue
    ...
    return jsonify({'error': f'3회 시도 후 실패: {last_error}. 잠시 후 다시 시도해주세요.'}), 503
